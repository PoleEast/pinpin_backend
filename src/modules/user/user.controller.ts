...
import { ApiCookieAuth, ApiOperation, ApiResponse as SwaggerApiResponse, ApiTags } from "@nestjs/swagger";
import ApiCommonResponses from "../../common/decorators/api_responses.decorator.js";
import { AccountRequest, ApiResponse, LoginResponse } from "pinpin_library";
...
export class UserController {
  ...
  @Post("register")
  async register(@Body() registerDto: RegisterDto, @Res({ passthrough: true }) response: Response): Promise<ApiResponse<LoginResponse>> {
  ...
  @Post("login")
  async Login(@Body() loginDto: LoginDto, @Res({ passthrough: true }) response: Response): Promise<ApiResponse<LoginResponse>> {
  ...
  @UseGuards(JwtGuard)
  @Get("check-auth")
  async CheckAuth(@GetUser() user: User): Promise<ApiResponse<LoginResponse>> {
    return {
      statusCode: HttpStatus.OK,
      message: "授權成功",
      data: {
  ...
  @UseGuards(JwtGuard)
  @Patch("updateUser")
  async updateUser(@GetUser() user: User, @Body() accountDto: AccountDto): Promise<ApiResponse<AccountRequest>> {
